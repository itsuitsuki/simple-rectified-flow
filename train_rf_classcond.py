...
import os
from rectified_flow import ClassConditionalRectifiedFlow
from skipconn import ClassConditionalUNet
...
rf = ClassConditionalRectifiedFlow(unet=ClassConditionalUNet(1, num_classes=10, num_hiddens=64).to(device), num_ts=300).to(device)
...
for epoch in range(n_epochs):
    ...
    for i, (x, y) in loop:
        ...
        optimizer.zero_grad()
        loss = rf.forward(x.to(device), y.to(device))
        loss.backward()
        with torch.no_grad():
            if trainstep % 10 == 0:
    ...
    with torch.no_grad():
        if epoch in epochs_to_sample:
            ...
            for j in range(4):
                for i in range(10):
                    x, anim = rf.sample(torch.tensor([i]).to(device), (28, 28), guidance_scale=5.0, seed=i*4+j)
                    canvas[j*28:(j+1)*28, i*28:(i+1)*28] = x.squeeze()
                    animation_caches_each_sample[i].append(anim)
            plt.imshow(canvas.cpu().numpy(), cmap="gray")
            plt.axis("off")
            plt.tight_layout()
            os.makedirs("output/classcond", exist_ok=True)
            # plt.savefig(f"output/classcond/epoch_{epoch+1}_sample_results.png", bbox_inches='tight', pad_inches=0, dpi=450)
            media.write_image(f"output/classcond/epoch_{epoch+1}_sample_results.png", canvas.cpu().numpy())
            # plt.show()
            ...
            animation_output_path = f"output/classcond/epoch_{epoch+1}_animation.gif"
...
plt.tight_layout()
plt.savefig("output/classcond/training_loss_curve.png", bbox_inches='tight', pad_inches=0, dpi=450)